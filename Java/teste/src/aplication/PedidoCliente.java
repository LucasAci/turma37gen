		...
		do {
			...
			if (op == 'S') {
				do {
					...
					for (int x = 0; x < vitrine.size(); x++) { // VERIFICA SE O CÓGIDO INFORMADO É IGUAL À ALGUM DOS
																// EXISTENTES NO MENU E GUARDA TAL POSIÇÃO NA LISTA
					...
					if (pos >= 0) {
						...
						if (qtdCar > 0 && qtdCar <= vitrine.get(pos).getEstoque()) {
							carrinho.entraCarrinho(vitrine.get(pos).getNome(), codCar, vitrine.get(pos).getValor(),
									qtdCar);
